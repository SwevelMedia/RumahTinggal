...
class PembelianModel extends CI_Model
{
  ...
  function getRumah($id)
  {
    ...
    return $this->db->query("SELECT rumah.id_rumah,nama_rumah,luas_bangunan,COALESCE(a.lantai, 1) AS lantai FROM rumah LEFT JOIN (SELECT id_rumah,MAX(lantai) AS lantai FROM denah GROUP BY id_rumah) a ON rumah.id_rumah=a.id_rumah WHERE rumah.id_rumah={$id}");
  }
  ...
  function simpanPembelianGratis($data)
  {
  ...
  function ubahAlamatCustomer($data)
  {
    ...
    $this->db->where('id_customer', $id_customer)->update($this->tabel_customer, $value);
  }
}
